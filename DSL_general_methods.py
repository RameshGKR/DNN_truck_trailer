...
def Dagger(train_NN, simulate_system_traces, validate_datasets, validate_trace_datasets, expert_policy, start_dataset_csv, start_point_dataset_csv, expert_trace_dataset_csv, Dagger_loops, trace_length, p, output_map, starting_index):
    total_dataset = DSL_Data_Set()
    total_dataset.initialize_from_csv(start_dataset_csv)
    ...
    for idx in range(Dagger_loops):
        iteration_output_map = output_map+"\iteration_"+str(idx+starting_index+1)
        ...
        beta = math.pow(p, idx+starting_index)
        loop_policy = expert_policy.statistical_combination([trained_policy],[beta,(1-beta)])

        trace_dataset = simulate_system_traces.simulate_system_traces(loop_policy, start_point_dataset.input, trace_length)
